 ...
 * A sentence with a ___ is shown. Everyone types their funniest answer.
 * Answers revealed anonymously, everyone votes for the best.
 * Points: 25 participation + 75 per vote received + 100 bonus for top answer.
 *
 * Phases: input (30s) → vote (20s) → results → (next prompt / end round)
...
export default function FillGapScreen() {
  ...
  async function handleSubmit() {
    ...
    setSubmitted(true)
    await submitFillAnswer(gameCode, myId, myAnswer)
  }
  ...
  // Format prompt: replace ___ with highlighted blank
      ...
      <span>
        {parts.map((part, i) => (
          <span key={i}>
            {part}
            {i < parts.length - 1 && (
              <span style={{ display: 'inline-block', minWidth: 80, borderBottom: '3px solid var(--accent)', margin: '0 4px', verticalAlign: 'bottom', opacity: 0.7 }}>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
            )}
          </span>
        ))}
      </span>
  ...
  const topAnswerIds = tally ? Object.entries(tally).filter(([, c]) => c === maxVotes && c > 0).map(([id]) => id) : []
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '1.05rem', fontWeight: isTop ? 700 : 400, marginBottom: 6, lineHeight: 1.35 }}>
                        "{answer}"
                        {isTop && voteCount > 0 && <span style={{ marginLeft: 8, fontSize: '1.2rem' }}>🏆</span>}
                      </div>
                      {player && (
